---
# title: "Publications"
format: 
  html:
    keep-md: true
jupyter: python3
echo: false
---


```{python}
#| output: asis

import bibtexparser
from urllib.parse import quote, urlparse

def format_authors(authors_raw, highlight_names=("Liu, Yangyuanchen", "Yangyuanchen Liu")):
    """
    Convert authors' full names into initials format and bold the specified name.
    """
    def normalize_name(name):
        parts = name.strip().split(", ")
        if len(parts) == 2:
            lastname, firstname = parts
            return f"{firstname} {lastname}".lower()
        return " ".join(name.strip().split()).lower()

    highlighted = {normalize_name(name) for name in highlight_names}
    formatted_authors = []
    authors = authors_raw.split(" and ")  # Split multiple authors
    for author in authors:
        parts = author.split(", ")
        if len(parts) == 2:  # Format: Lastname, Firstname
            lastname, firstname = parts
            initials = " ".join(f"{name[0]}." for name in firstname.split())  # Extract initials
            formatted_name = f"{initials} {lastname}"
        else:
            # Handle cases without a comma (Firstname Lastname)
            name_parts = author.split()
            lastname = name_parts[-1]
            firstname_parts = name_parts[:-1]
            initials = " ".join(f"{name[0]}." for name in firstname_parts)
            formatted_name = f"{initials} {lastname}"
        
        # Bold the specified name
        if normalize_name(author) in highlighted:
            formatted_name = f"**{formatted_name}**"
        formatted_authors.append(formatted_name)
    return ", ".join(formatted_authors)

def generate_doi_badge(doi):
    """
    Generate a clickable DOI badge using shields.io.
    """
    doi = doi.strip()

    doi_for_badge = quote(doi, safe="")
    badge_url = f"https://img.shields.io/static/v1?label=DOI&message={doi_for_badge}&color=blue"
    doi_url = f"https://doi.org/{doi}"

    return f"[![DOI: {doi}]({badge_url})]({doi_url})"

def generate_url_badge(url):
    """
    Generate a clickable URL badge using shields.io.
    """
    url = url.strip()

    parsed = urlparse(url)
    display_url = parsed.netloc if parsed.netloc else url.split("/")[0]
    display_url_for_badge = quote(display_url, safe="")

    badge_url = f"https://img.shields.io/static/v1?label=URL&message={display_url_for_badge}&color=blue"

    return f"[![URL: {display_url}]({badge_url})]({url})"

# Start Content

print("## Publications and Preprints")

# Load the BibTeX file
with open("./publications/papers.bib", "r", encoding="utf-8") as bibtex_file:
    paper_database = bibtexparser.load(bibtex_file)

# debug
# print("N entries =", len(paper_database.entries))
# print("Last 5 IDs =", [e.get("ID") for e in paper_database.entries[-5:]])
# print("Has liu2026gino =", any(e.get("ID") == "liu2026gino" for e in paper_database.entries))


with open("./publications/conferences.bib", "r", encoding="utf-8") as bibtex_file:
    pre_database = bibtexparser.load(bibtex_file)

# Sort entries by year in descending order
sorted_papers = sorted(
    paper_database.entries,
    key=lambda x: int(x.get("year", 0)), 
    reverse=True
)

sorted_pres = sorted(
    pre_database.entries,
    key=lambda x: int(x.get("year", 0)), 
    reverse=True
)


# Extract and display publications
for entry in sorted_papers:
    title = entry.get("title", "No Title")
    author = entry.get("author", "No Author")
    year = entry.get("year", "No Year")
    journal = entry.get("journal", "")
    volume = entry.get("volume", "")
    number = entry.get("number", "")
    pages = entry.get("pages", "")
    doi = entry.get("doi", "")
    url = entry.get("url", "")
    abstract = entry.get("abstract")

    # Format and display each publication
    print(f"**{title}** ({year}). _{format_authors(author)}_. _{journal}_ {volume}, {pages}.")
    if doi:
        print(generate_doi_badge(doi))
    elif url:
        print(generate_url_badge(url))
    if abstract:
        print(f"""
<details>
<summary>Abstract</summary>
{abstract}
</details>
        """)
    print()

print("## Presentations")

# Extract and display publications
for entry in sorted_pres:
    title = entry.get("title", "No Title")
    author = entry.get("author", "No Author")
    eventtitle = entry.get("eventtitle", "No Event")
    location = entry.get("location", "No Location")
    year = entry.get("date", "No Year")
    note_raw = entry.get("note")
    note = f" ({note_raw})" if note_raw else ""
    url = entry.get("url")
    if url:
        eventtitle_fmt = f"[{eventtitle}]({url})"
    else:
        eventtitle_fmt = eventtitle

    # Format and display each publication
    print(f"**{title}** ({year}). _{format_authors(author)}_. _{eventtitle_fmt}_{note}, {location}.\n")
```

